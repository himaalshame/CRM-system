...
import type { DashboardRecentOrder } from "../../services/dashboard.service";
            ...
            {orders.map((order) => (
              <TableRow key={order.id}>
                ...
                <TableCell className="py-3 text-gray-700 dark:text-gray-300">
                  {order.orderItems.map((item) => item.service.name).join(", ") || "Service"}
                </TableCell>
                <TableCell className="py-3 text-gray-700 dark:text-gray-300">
                  {order.totalPrice}
                </TableCell>
                <TableCell className="py-3">
                  <Badge color={order.status === "COMPLETED" ? "success" : order.status === "CANCELLED" ? "error" : "warning"}>
                    {order.status}
                  </Badge>
                </TableCell>
              </TableRow>
